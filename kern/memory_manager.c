...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	uint32* PageTableAddress = kmalloc(PAGE_SIZE);
	uint32* CurrentAddress = PageTableAddress;
	...
	for(int i=0;i<(PAGE_SIZE)/sizeof(uint32);++i,++CurrentAddress)
	{
		*CurrentAddress = 0;
	}
	int PageTablePhysicalAddress = kheap_physical_address((unsigned int)PageTableAddress);
	PageTablePhysicalAddress |=PERM_USER;
	PageTablePhysicalAddress |=PERM_PRESENT;
	PageTablePhysicalAddress |=PERM_WRITEABLE;
	ptr_page_directory[PDX(virtual_address)] = PageTablePhysicalAddress;
	//	b.	clear the TLB cache (using "tlbflush()")
	tlbflush();
	return (void*)PageTableAddress;
...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	if(size % PAGE_SIZE != 0)++NumberofPages;

	for(int i=0;i<NumberofPages;++i,VirtualAddress+=PAGE_SIZE)
	{
		int ret = pf_add_empty_env_page(e, VirtualAddress, 0);
		if(ret != 0)
			panic("the page file is full");
	}
...
void __freeMem_with_buffering(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	for(int i=0;i<e->page_WS_max_size;++i)
	{
		...
		if(Address>= virtual_address && Address < virtual_address + size)
		{
			...
			struct Frame_Info* FrameInfoPointer = get_frame_info(e->env_page_directory, (void*)Address, &PageTablePointer);
	...
	for(int i=0;i<NumberofPages;++i,VirtualAddress+=PAGE_SIZE)
	{
		if(pt_get_page_permissions(e,VirtualAddress)&PERM_BUFFERED)
		{
			if(pt_get_page_permissions(e,VirtualAddress)&PERM_MODIFIED)
			{
				uint32* PageTablePointer = NULL;
				struct Frame_Info* FrameInfoPointer = get_frame_info(e->env_page_directory, (void*)VirtualAddress, &PageTablePointer);
				bufferlist_remove_page(&modified_frame_list,FrameInfoPointer);
				FrameInfoPointer->isBuffered = 0;
				FrameInfoPointer->environment = NULL;
				free_frame(FrameInfoPointer);
				PageTablePointer[PTX(VirtualAddress)] = 0;
			...
			else
			{
				...
				FrameInfoPointer->isBuffered = 0;
				FrameInfoPointer->environment = NULL;
				bufferlist_remove_page(&free_frame_list,FrameInfoPointer);
				free_frame(FrameInfoPointer);
				PageTablePointer[PTX(VirtualAddress)] = 0;
	...
	for(int i=0;i<NumberofPages;++i,VirtualAddress+=PAGE_SIZE)
	{
		...
		uint32* PageTablePointer = NULL;
		get_page_table(e->env_page_directory,(void*)VirtualAddress,&PageTablePointer);
		if(PageTablePointer != NULL)
		{
			int EmptyPageTableFlag = 1;
			for(int j=0;j<1024;++j)
			{
				if(PageTablePointer[j] != 0)
				{
					EmptyPageTableFlag = 0;
					break;
				}
			}
			if(EmptyPageTableFlag == 1)
			{
				kfree((void*)PageTablePointer);
				pd_clear_page_dir_entry(e,VirtualAddress);
			}
		}
	}
...
void bufferList_add_page(struct Linked_List* bufferList,struct Frame_Info *ptr_frame_info)
{
	...
	//cprintf("inserting frame_info %x, mod_first = %x, mod_last = %x\n", ptr_frame_info , LIST_FIRST(&), LIST_LAST(&modified_frame_list));
