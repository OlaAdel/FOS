...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	int id=allocate_share_object(&allocatedObject);
	if(id==E_NO_SHARE)
	{
		return E_NO_SHARE;
	}
	...
	for(int i=0 ; i<NumberOfAllocatedPages ; i++)
	{
		...
		map_frame(myenv->env_page_directory , ptr_frame_info ,(void*)CurrentAddress,PERM_WRITEABLE|PERM_USER);
		add_frame_to_storage(allocatedObject[0].framesStorage, ptr_frame_info ,i);
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	uint32 CurrentAddress = (uint32)virtual_address;
	int NumberOfAllocatedPages =shares[id].size/PAGE_SIZE;
	if(shares[id].size%PAGE_SIZE!=0)++NumberOfAllocatedPages;
	for(int i=0 ; i<NumberOfAllocatedPages; i++)
	{
		struct Frame_Info* frame=get_frame_from_storage(shares[id].framesStorage , i);
		if(shares[id].isWritable==0)
			map_frame(myenv->env_page_directory,frame,(void*)CurrentAddress,PERM_USER|PERM_PRESENT);
		else
			map_frame(myenv->env_page_directory,frame,(void*)CurrentAddress,PERM_USER|PERM_PRESENT|PERM_WRITEABLE);
		CurrentAddress+=PAGE_SIZE;
	}
